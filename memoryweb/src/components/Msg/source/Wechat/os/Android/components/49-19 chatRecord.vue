...
export default {
    ...
    computed: {
        ...
        url_pre() {
            return _.get(this.data, '$url_pre');
        },
        title() {
            return _.get(this.data, 'appmsg.title');
        },
        des() {
            const des = _.get(this.data, 'appmsg.recorditem.recordinfo.desc', '').replace(/(\r\n|\r|\n)/g, '<br/>');
            return des;
        },
        datalist() {
            return _.get(this.data, 'appmsg.recorditem.recordinfo.datalist.dataitem', []) || [];
        },
    },
    methods: {
        avatarUrl(v) {
