<template>
	...
	<div class="devFlag">
		...
		<el-tag v-if="msg.source === 'QQ' && /\[(?!(自动回复|Android|iPhone|IP地址)).+\]/.test(msg.html)">好像这是一条表情</el-tag>
		<!-- 【条件】的跳转按钮 -->
...
export default {
	...
	methods: {
		...
		goWechatLost(m) {
			...
			switch (m.type) {
				case '自定义表情':
					return !item.$url_emoji;
				// case '图片':
				// 	return !(Array.isArray(item.$imgUrl) ? item.$imgUrl[0] : item.$imgUrl);
				// case '视频':
				// 	return !item.$mp4info.mp4Url;
				// case '语音':
				// 	return !item.$mp3Info.mp3Url;
				default:
					return false;
			}
		},
		...
		goWechatVideo(m) {
			return m.source === 'Wechat' && m.type === '视频' && m.$Wechat.webData.$mp4info.mp4Url;
		},
		goQQImgProblemMsg(m) {
			return (
				m.source === 'QQ' &&
