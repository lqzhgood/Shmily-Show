...
function calc(directionType) {
    ...
    const countYearData = fs.readJsonSync(path.join(outDir, `./year-count.json`));
    const maxData = countYearData.reduce((pre, cV) => (cV.max >= pre.max ? cV : pre));

    const timeLineYearData = fs.readJsonSync(path.join(outDir, `./timeLine/year.json`));
    ...
    fs.writeFileSync(
        path.join(outDir, `./sourceCount.json`),
